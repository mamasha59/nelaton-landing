  ...
  return (
    <div className="flex flex-1 px-6 py-4 mt-2 bg-[#ffffffcf] backdrop-blur-lg rounded-xl justify-between items-center shadow-md lg:shadow-none relative z-30">
      ...
      <button
        ...
        className="px-4 py-2 rounded-[32px] lg:flex hidden gap-2 lg:gap-0 lg:bg-black bg-white items-center"
      >
        {/* BIG SCREENS */}
        <p className="text-base text-white uppercase mr-[2px] font-semibold">
          {locale}
        </p>
        <Image
      ...
      {whetherShowMenu && (
        <button
          title="burger menu(features-faq-contacts)"
          aria-label="burger menu(features-faq-contacts)"
          onClick={handleMenu}
          className="flex lg:hidden items-center justify-center relative w-6 h-6"
        >
          <div
            className={`w-6 h-[2px] bg-black absolute transition-transform duration-300 ease-in-out ${
              openMenu ? "rotate-45" : "-translate-y-[6px]"
            }`}
          />
          <div
            className={`w-6 h-[2px] bg-black absolute transition-opacity duration-300 ease-in-out ${
              openMenu ? "opacity-0" : ""
            }`}
          />
          <div
